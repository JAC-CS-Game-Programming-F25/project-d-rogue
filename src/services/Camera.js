...
export default class Camera {
    ...
    initMouseEvent() {
        canvas.addEventListener("mousemove", (event) => {
            ...
            this.mouse.x = screenX + this.position.x;
            this.mouse.y = screenY + this.position.y;
        });
    }
    ...
    update(dt) {
        ...
        if (input.isMouseButtonPressed(Input.MOUSE.LEFT)) {
            const dx = this.mouse.x - playerCenter.x;
            const dy = this.mouse.y - playerCenter.y;
        ...
        this.lastGroundedY = this.player.position.y;
        ...
        this.position.x = Math.max(
            0,
            Math.min(
                this.worldWidth - this.viewportWidth / this.scale,
                this.position.x
            )
        );
        ...
        this.position.x = Math.round(this.position.x - 100);
        this.position.y = Math.round(this.position.y - 100);
    }
